...
double vbfConversionFactor(string taupt, string isoregion) {
	...
	if (inputfile->IsZombie()) cout << "ERROR: in vbfConversionFactor couldn't open allQCD file for taupt" << taupt << endl;
	TH1F* h_ditaucharge;
...
double vbfConversionFactor_statunc(string taupt, string isoregion) {
	...
	TFile *inputfile = TFile::Open((taupt + "/allQCD_"+ taupt +".root").c_str());
	if (inputfile->IsZombie()) cout << "ERROR: in vbfConversionFactor_statunc couldn't open allQCD file for taupt" << taupt << endl;
	TH1F* h_ditaucharge;
	TH1F* h_ditauchargeVBFinverted;
...
double vbfConversionFactorMCSyst(string taupt, string isoregion, double variation) {
	...
	TFile *inputfile = TFile::Open((taupt + "/allQCD_"+ taupt +".root").c_str());
	if (inputfile->IsZombie()) cout << "ERROR: in vbfConversionFactorMCSyst couldn't open allQCD file for taupt" << taupt << endl;
...
TH2F* makeEffPlot(string isoregion, TFile *inputfile) {
...
TH2F* makeEffPlotStatUnc(string isoregion, TFile *inputfile) {
...
//uncertainty plot for a given signal sample and selection
TH2F* makeEffPlotSyst(string isoregion, double variation,TFile *inputfile) {
	...
	h2_DiJetInvMass_vs_MET_eff_syst = new TH2F ("h2_DiJetInvMass_vs_MET_eff_systunc","h2_DiJetInvMass_vs_MET_eff_syst", nbinsx, 0., 240., nbinsy , 0., 2500.);
...
TH2F* makeBackgroundPlot_LtoT_StatUnc(string taupt, string isoregion){
	...
	TFile *inputfile = TFile::Open((taupt + "/allQCD_"+ taupt +".root").c_str());
	if (inputfile->IsZombie()) cout << "ERROR: couldn't open allQCD file for taupt" << taupt << endl;
...
void makeXSectionUnc(string taupt,string chi, string stau, string lsp, double lumi, bool debug) {
	...
	if (inputfile->IsZombie()) cout << "ERROR: couldn't open file for chi" << chi << " stau" << stau << " lsp" << lsp << endl;
  ...
  TH2F* h2_DiJetInvMass_vs_MET_background;
	...
	h2_DiJetInvMass_vs_MET_eff_signal = makeEffPlot("Taui2TightIso", inputfile);
	h2_DiJetInvMass_vs_MET_eff_signal_stat = makeEffPlotStatUnc("Taui2TightIso", inputfile);
	h2_DiJetInvMass_vs_MET_eff_signal_mcsystup = makeEffPlotSyst("Taui2TightIso", 0.5, inputfile);
	h2_DiJetInvMass_vs_MET_eff_signal_mcsystdown = makeEffPlotSyst("Taui2TightIso", -0.5, inputfile);
  ...
  h2_DiJetInvMass_vs_MET_xsec = new TH2F (("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt).c_str(),("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt).c_str(), nbinsx, 0., 240., nbinsy , 0., 2500.);
  ...
  TH2F* h2_DiJetInvMass_vs_MET_xsec_stat;
	...
	h2_DiJetInvMass_vs_MET_xsec_stat = new TH2F (("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_stat").c_str(),
																							 ("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_stat").c_str(),
																							 nbinsx, 0., 240., nbinsy , 0., 2500.);
	h2_DiJetInvMass_vs_MET_xsec_mcsystup = new TH2F (("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_mcsystup").c_str(),
																									 ("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_mcsystup").c_str(),
																									 nbinsx, 0., 240., nbinsy , 0., 2500.);
	h2_DiJetInvMass_vs_MET_xsec_mcsystdown = new TH2F (("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_mcsystdown").c_str(),
																										 ("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_mcsystdown").c_str(),
																										 nbinsx, 0., 240., nbinsy , 0., 2500.);
	h2_DiJetInvMass_vs_MET_xsec_vbfsystup = new TH2F (("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_vbfsystup").c_str(),
																										("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_vbfsystup").c_str(),
																										nbinsx, 0., 240., nbinsy , 0., 2500.);
	h2_DiJetInvMass_vs_MET_xsec_vbfsystdown = new TH2F (("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_vbfsystdown").c_str(),
																											("JetInvMass_vs_MET_xsec_" + chi + "_" + stau + "_" + lsp + "_" + taupt + "_vbfsystdown").c_str(),
																											nbinsx, 0., 240., nbinsy , 0., 2500.);
  ...
  for (int i = 0; i < nbinsx; i++) {
	...
	if (debug) cout << "Clearing memory..." <<endl;
	//clearing memory
	//inputfile->Delete();
//
